...
model = STSG(data, time_fmt='%d/%m/%Y')

#%%
model.plot_fourier(m_values=[3,50,100], n_samples=100, non_negative=True)
plt.savefig(os.path.join(export_dir, f'{data_name} fourier w diff coeff.'), bbox_inches='tight')


#%% Fourier
# Generate data using Fourier
synth_data_fourier = model.gen_fourier(keep_phases=100, n_samples=1000, non_negative=True)
...
#%% CoSMoS
# Import from R
df = pd.read_csv(f'{export_dir}\\NO_cosmos.csv')
...
cosmos_wass = model.wass_dist(synth_data_cosmos, dec_places=3)
...
# statistics
cosmos_stats = model.calculate_statistics(synth_data_cosmos, dec_places=3)
...
GAN_path = f'{os.getcwd()}\\GAN\\{data_name} Generator\\NO_epochs=1000_seq=30_batch=5.pth'
...
GAN_wass = model.wass_dist(synth_data_GAN, dec_places=3)
...
# statistics
GAN_stats = model.calculate_statistics(synth_data_GAN, dec_places=3)

...
# synth_data_fourier = model.gen_fourier(keep_phases=5, n_samples=2)
synth_data_fourier = model.gen_fourier(keep_phases=5, n_samples=1000)
print('Synth. done')
...
fourier_wass = model.wass_dist(synth_data_fourier, dec_places=dec_places)
...
# statistics
fourier_stats = model.calculate_statistics(synth_data_fourier, dec_places=dec_places)
...
synth_data_ARIMA = model.gen_ARIMA(p=6, d=0, q=10, n_samples=1000)
...
ARIMA_wass = model.wass_dist(synth_data_ARIMA, dec_places=dec_places)
...
#statistics
ARIMA_stats = model.calculate_statistics(synth_data_ARIMA, dec_places=dec_places)
ARIMA_stats.to_csv(os.path.join(export_dir, f'{data_name}_ARIMA_stats.csv'))

#%% CoSMoS
# Import from R
df = pd.read_csv(f'{export_dir}\\{data_name}_cosmos.csv')
...
cosmos_wass = model.wass_dist(synth_data_cosmos, dec_places=dec_places)
...
# statistics
cosmos_stats = model.calculate_statistics(synth_data_cosmos, dec_places=dec_places)
...
GAN_path = f'{os.getcwd()}\\GAN\\{data_name} Generator\\{data_name}_epochs=500_batch=1_seq=127'
...
scaler.fit_transform(model.data.values.reshape(-1,1))[:,0]
...
synth_data_GAN.to_csv(os.path.join(export_dir, f'{data_name}_GAN.csv'))
#%% GAN Analysis
# similarity scores

GAN_dtw = model.dtw_score(synth_data_GAN)
GAN_wass = model.wass_dist(synth_data_GAN,dec_places=dec_places)
...
# statistics
GAN_stats = model.calculate_statistics(synth_data_GAN,dec_places=dec_places)
...
#%%
model.plot_fourier(m_values=[5,30,100], n_samples=100)
